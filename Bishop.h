...
inline Bishop :: Bishop(PieceColor color)    {
    this->color = color;
}

inline PieceColor Bishop :: getColor()   const {
    return color;
}

inline Move* Bishop :: getLegalMoves(int row, int col, const Board& board, int& count)    {
    // max moves for Bishop 13
    Move* temp = new Move [13];
    count = 0;
